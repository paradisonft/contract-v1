...
import Paradiso.Contracts.FixedPrice.Types (FixedPriceSale(..))
...
main = do
  ...
  let assetPrice        = read price'
      sellerHash        = fromString seller'
      tn                = fromString tn'
      cs                = fromString cs'
      royaltyHash       = fromString raddr'
      royaltyPercentage = read rprct'
      sale   = FixedPriceSale sellerHash cs tn assetPrice royaltyHash royaltyPercentage

  writeData ("datum-" ++ show cs ++ "-" ++ tn' ++ ".json") sale
  putStrLn "Done"
